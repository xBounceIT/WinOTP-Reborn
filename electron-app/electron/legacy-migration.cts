...
function mapLegacySettings(source) {
  ...
  return normalizeSettings({
    ...
    remotePin: asBoolean(input.IsWindowsHelloRemotePinEnabled, defaultElectronSettings.remotePin),
        ...
        try {
          ...
          if (!imported?.success) {
            state.appLock = {
              ...
              issueCount: issueCount + 1,
              message:
                imported?.message ?? "The native app-lock credentials could not be migrated.",
            };
          } else {
            state.appLock = {
              ...migrationPartDefaults,
...
function runLegacyMigration(app, options: any = {}) {
  ...
  migrateSettings({
    ...
    state,
    legacySettingsPath:
      options.legacySettingsPath ?? path.join(directoryPath, LEGACY_SETTINGS_FILE_NAME),
