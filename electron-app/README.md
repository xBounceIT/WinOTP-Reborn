...
This is the Electron application for WinOTP. The React renderer is TypeScript/TSX; Electron main-process, preload, test, and build-boundary code is plain CommonJS/ESM JavaScript. The Rust `winotp-core` sidecar provides portable domain logic. `build:electron` copies the JavaScript shell into the ignored `electron-dist/` directory.
...
The renderer uses the native bridge for account listing, add/edit/delete, usage counters, encrypted backup import/export, and Rust-backed domain operations. The main process delegates account normalization, TOTP generation, imports, ordering, backup encryption, and portable policy decisions to Rust. Browser-only work such as `jsqr` remains in the renderer. Screen-region QR capture uses Electron's cross-platform `screen` and `desktopCapturer` APIs. It creates one local-coordinate overlay per OS display, preserves negative origins and per-monitor DPI, matches sources by `display_id`, and adds decoded `otpauth://` accounts to the local account list.
...
The development bridge copies the JavaScript Electron boundary before launching. The Rust sidecars can be compiled with Cargo on demand. Packaging automatically builds both platform-specific sidecars into `native/`; build them separately when needed:
